...
const Marketplace: React.FC = () => {
    ...
    useEffect(() => {
        if (selectedFarm && hbarToUsdRate > 0) {
            ...
            const commission = totalHbar * 0.02;
            ...
            {selectedFarm && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 animate-fade-in">
                    <div className="bg-white rounded-lg shadow-2xl p-8 max-w-md w-full m-4">
                        <h2 className="text-2xl font-bold mb-4">Purchase from {selectedFarm.name}</h2>
                        <p className="mb-2 text-text-secondary">Available: {selectedFarm.availableTons} credits.</p>
                        ...
                        <div className="bg-gray-100 p-4 rounded-lg mt-6 text-sm space-y-2">
                            <div className="flex justify-between">
                                <span className="text-text-secondary">Farmer Receives (98%):</span>
                                <span className="font-semibold text-text-primary">~ {purchaseDetails.farmerReceives.toFixed(4)} ℏ</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-text-secondary">Platform Commission (2%):</span>
